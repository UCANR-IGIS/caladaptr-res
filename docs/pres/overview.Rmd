...
cap1_db <- ca_getvals_db(cap1,
                         db_fn = "c:/data/mydata.sqlite",
                         db_tbl = "temp_min_max")
```

- downloaded values will be saved into SQLite database  
- slow connection? No problem - it will chug away on its own   
- disconnected? No problem - it will pick up where it left off     
- additional functions help you view contents of the SQLite database, manage indices, etc.

![](images/dbplyr_240x277.png)

# Need Rasters?

No problem! Data can be downloaded as stars.

```{r cap1_rst, eval = FALSE}
cap1_tifs <- ca_getrst_stars(cap1,
                             sidecar_write = TRUE)
```

- climate variables are downloaded as TIFs  
- sidecar file saves the temporal details  
- read tifs back into R with `ca_read_stars()`   

# Shiny Apps

![](images/shiny-hex_87x100.png) &rarr; browser based GUIs
